      ...
      if (!empty($vars['content'])) {
        foreach ($vars['content'] as $key1 => $result) {
          ...
          if (isset($result->node_data_field_image_field_image_fid)) {
            // get image id
            $fid = $result->node_data_field_image_field_image_fid;//node_data_field_pager_item_text_field_image_fid;
            // get path to image
...
function zeneurami_preprocess_node(&$vars) {
  // set node & check for content type preprocessor
  $function = 'zeneurami_preprocess_node_' . $vars['node']->type;
  if (function_exists($function)) {
      $function($vars);
  }
}

function zeneurami_preprocess_node_advpoll_binary(&$vars) {
  $vars['content'] = str_replace(
    array('<label>Description: </label>',
          'Accreditation poll')
    , '', $vars['content']);
}